...
```{r prepData, , echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
library(tidyverse)
library(plotly)
...
sort(unique(formicidae$collector))
formicidaeCleaned <- formicidae %>% 
  mutate(collector = case_when(`Collectors - Last Name` == "C. Saux" ~ "C. S. Moreau",
                               ...
                               `Collectors - Last Name` == "JC Bradley" ~ "J. C. Bradley",
                               `Collectors - Last Name` == "J.C. Bradley" ~ "J. C. Bradley",
...
## Exploring the Cornell University Insect Collection's ant specimens

Who has collected the most ants, and when did they do it?

```{r collectors, echo=FALSE, warning=FALSE}
...
How have collecting locations changed over time?

```{r decades, echo=FALSE, warning=FALSE}
...
ggplot(data = formicidaeCleaned) +
  ...
  theme(axis.text.x = element_text(angle = 90,
                                   hjust = 1)) +
  theme_bw() +
  ggtitle("CUIC ant collecting locations over time") + 
  scale_fill_pomological()
  
```

Which countries have the most ants collected from them?
...
ggplot(data = formicidaeCleaned) +
  ...
  theme(axis.text.x = element_text(angle = 45,
                                   hjust = 1)) +
  ggtitle("CUIC ant specimens by country") + 
  scale_fill_pomological()
```

How are CUIC ants distributed across the globe?
...
collectingLocations <- ggplot() +
  ...
  scale_color_gradientn(colors = c("#df6322", 
                                   ...
                                   "#529a92")) +
  theme_bw() +
  ggtitle("CUIC ants around the world")
